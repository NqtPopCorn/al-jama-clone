        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <Badge variant={isDark ? 'secondary' : 'outline'} className="font-mono text-xs font-bold">
                {projectDetails.key}
              </Badge>
            ...
            <p className={`max-w-2xl text-xs leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              {projectDetails.description || 'Enterprise requirements baseline and traceability project.'}
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className={`text-2xl font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {statusBreakdown.find((s: any) => s.status?.toLowerCase() === 'approved')?.count || 0}
              </div>
              <div className={`text-[11px] font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                Approved Items
              </div>
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className={`text-2xl font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {statusBreakdown.find((s: any) => s.status?.toLowerCase().includes('review'))?.count || 0}
              </div>
              <div className={`text-[11px] font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                In Review
              </div>
