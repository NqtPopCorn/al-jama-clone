...
_(Đã cập nhật theo phản hồi khách hàng - 09/09/2026)_

| #   | Câu hỏi / Rủi ro cần xác nhận                                                                                                                                                                       | Trạng thái                | Nội dung xác nhận / giả định                                                                                            |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 1   | Module Test Management (Chương 6.4) có thực sự cần trong MVP giai đoạn 1, hay có thể dời sang giai đoạn 2 để tập trung nguồn lực cho luồng Item – Traceability – Review trước?                      | ✅ Đã xác nhận            | **Không** — chưa cần ở giai đoạn sớm; ưu tiên nguồn lực cho luồng Item – Traceability – Review trước.                   |
| 2   | Chữ ký điện tử (electronic signature) trong MVP chỉ cần mô phỏng UI/UX hay phải đảm bảo giá trị pháp lý tương đương tiêu chuẩn ngành (ví dụ 21 CFR Part 11)?                                        | ✅ Đã xác nhận (một phần) | **Implement đơn giản** (mô phỏng UI/UX) — thực hiện ở **giai đoạn cuối của MVP**, không phải ngay từ Phase 2.           |
| 3   | Cơ chế phân quyền theo loại license (Creator/Reviewer đầy đủ vs. Reviewer hạn chế) có cần triển khai đầy đủ như Jama Connect, hay MVP dùng mô hình phân quyền đơn giản hoá (ví dụ chỉ 2-3 vai trò)? | ⏳ Chưa xác nhận          | Giữ giả định cũ: 2 license (`full`, `reviewer_limited`), 2 project_role, review_role tách riêng theo từng review.       |
| 4   | Tính năng import item từ file Word/Excel có bắt buộc trong MVP hay giai đoạn đầu chỉ cần nhập tay và reuse item có sẵn?                                                                             | ✅ Đã xác nhận            | **Có trong MVP**, nhưng thực hiện ở **giai đoạn cuối của MVP** — đầu MVP chỉ nhập tay + reuse item có sẵn.              |
| 5   | Danh sách cụ thể các loại item cần hỗ trợ (ví dụ: Requirement, Use Case, Test Case, Defect, Epic...) và các trường dữ liệu tương ứng cho từng loại là gì?                                           | ✅ Đã xác nhận            | **3 loại**: Requirement, Use Case, Test Case — mỗi loại field tối thiểu. **Không có Defect, Epic** ở giai đoạn MVP này. |
| 6   | "Rolling review" dựa trên filter + workflow status — mức độ phức tạp của bộ lọc/workflow builder cần có trong MVP là gì, hay dùng bộ lọc/trạng thái mặc định dựng sẵn?                              | ⏳ Chưa xác nhận          | Giữ giả định cũ: dùng filter/trạng thái dựng sẵn, không xây filter builder tuỳ biến.                                    |
| 7   | Hệ thống gửi email thật (SMTP tích hợp) hay chỉ cần mô phỏng/log thông báo trong phạm vi môi trường demo & UAT?                                                                                     | ⏳ Chưa xác nhận          | Giữ giả định cũ: SMTP thật + log song song để debug/demo.                                                               |
| 8   | Có cần hỗ trợ đa dự án, đa tổ chức (multi-tenant) ngay từ MVP, hay MVP chỉ cần phục vụ một tổ chức/một bộ dữ liệu demo?                                                                             | ✅ Đã xác nhận            | **Không** — single-tenant per deployment, đúng theo giả định A1 (Database Schema).                                      |
